...
def dict_wells_corr(F_dir,conn):
    ...
    list_files = list(filter(lambda file: ".csv" in file, listdir(F_dir)))
    data_raw = pd.DataFrame()
    for file in list_files:
        tmp_data = pd.read_csv(os.path.join(F_dir,file), sep=",")
...
def cell_cycle_summary(data_dir,conn):
    ...
    """
    data_IF, data_thresholds = assign_cell_cycle_phase(dict_wells_corr(data_dir,conn=conn))
...
if __name__ == '__main__':
    ...
    conn.connect()
    df=dict_wells_corr(F_dir='/Users/haoranyue/Desktop/221102_CellCycleProfile_Exp5_inhibitors_RPE1cdk1as/',conn=conn)
    df_2=assign_cell_cycle_phase(df,"experiment", "plate_id", "well", "well_id", "image_id",
