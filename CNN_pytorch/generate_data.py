...
class Image:
    ...
    def __init__(self, well, omero_image, meta_data, exp_paths, flatfield_dict):
        ...
        self.image_ID = omero_image.getId()
        ...
        self.nuclei_list=self._get_data(width=20)
...
if __name__ == "__main__":
    ...
    @omero_connect
    def extraction_data(conn=None):
        meta_data = MetaData(928, conn)
        exp_paths = ExpPaths(meta_data)
        well = conn.getObject("Well", 9651)
        print(well.getImage(8).getId())
        omero_image = well.getImage(8)
        flatfield_dict = flatfieldcorr(well, meta_data, exp_paths)
        imgs = Image(well, omero_image, meta_data, exp_paths, flatfield_dict)
